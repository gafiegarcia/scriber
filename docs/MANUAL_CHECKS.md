...
- Watch a whole successful dictation and count the animations. Exactly one is Scriber's: the pill fading in as recording starts. Everything after it — **Transcribing…**, the result, and the pill leaving — appears and goes with no transition of Scriber's own. The dismissal still carries AppKit's short fade for a utility window, which is expected; what matters is that it begins the instant the text is delivered rather than lingering. Any pill other than a dictation's first that eases in is this rule coming undone.
