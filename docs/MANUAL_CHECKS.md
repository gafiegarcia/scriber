...
- Start a hands-free dictation and open Settings with Command-comma while it runs. **Look before you stop it:** on General, every preset, the recorded chord, **Record**, and **Redo Setup** are greyed, and on Sound, **Check Input Level** is; none of them answers a click. Then press **Redo Setup…** with nothing running, and start a dictation with the confirmation on screen: the box closes on its own rather than leaving a prominent button that dismisses and does nothing. Nothing opens, nothing flashes, and the pill's timer and waveform stay smooth. Only then stop the dictation, and watch every control come back. Escape ends it before transcription, so this spends no API credit.
- Then open the main window mid-dictation from the menu bar item's **Open Scriber**, keep talking, switch back to where the text is going, and stop. The recording survives the window opening and the whole transcript lands at the cursor, including what was said while the window came up. **Spends API credit.** Settings is the route that historically cancelled a dictation and is checked above on its own, which is why the two are run separately rather than trusted together. The toolbar's warning control is not a third route: it exists only while a recovery condition does, and every one of those either stops the dictation starting or fails its transcription.
