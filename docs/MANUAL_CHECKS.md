...
- Watch a whole successful dictation and count the animations. There is exactly one: the pill fading in as recording starts. Everything after it — **Transcribing…**, the result, and the pill leaving — appears and goes with no transition at all, and the dismissal is instant rather than quick. A pill that fades out, or any pill other than a dictation's first that eases in, is this rule coming undone.
- Cancel mid-transcription so **Recover canceled dictation?** appears, then press **Recover**. As the panel shrinks to the one-line **Transcribing…**, it is a clean capsule from the first frame — never stretched, crooked, or squared-off, even for an instant. Run it again but start a fresh dictation from that panel instead of recovering: the same crossing, and it must look the same. **This spends API credit; ask first.**
