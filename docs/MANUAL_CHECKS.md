...
- Walk it on the shortest display available — the most scaled option under **Displays**, which is the setting someone chooses for their eyesight. The window is centred and wholly visible with its footer above the Dock, it never grows or shrinks as the steps change, and a step too tall for it scrolls. Change the display setting while setup is open: the window resizes to fit rather than keeping the height the previous display could take.
