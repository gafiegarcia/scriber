...
- Select a Bluetooth input in Settings ▸ Sound, disconnect it, then press the shortcut. **Dictation failed** appears as a box with no Retry and no See History, and History holds no row for the attempt — that pair is what this check answers, whatever the message says. Spends no credit; nothing is sent. Reaching a macOS-worded message is a separate and much harder aim: the device has to go while the capture session is opening, and disconnecting it earlier lands on Scriber's own "No recording is active." instead. Do not treat a Scriber-worded message as a failed run. Spec: Delivery and floating pill — "A notice never offers a recovery it cannot perform"
