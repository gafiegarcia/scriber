...
- While a dictation runs, try everything Settings should have grayed out: on General every preset, the recorded chord, **Record**, and **Redo Setup**; on Sound, **Check Input Level**. Then end the dictation. Spends no credit. Spec: Shortcuts and job lifecycle — "No control may change the dictation shortcut while a dictation is running"
...
- Click a text field level with a recording pill and a centimeter to either side of it. The click reaches that field, not the pill's window. Spec: Delivery and floating pill — "Show a floating pill at the bottom center"
...
- On a window whose size is fixed, check **Window ▸ Center** is enabled rather than grayed, then press ⌃🌐C with a confirmation on screen, where the default button is the one that confirms. Spec: Identity and workspace boundary — "a disabled menu item does not consume its key"
...
- Turn **Mute other audio while dictating** on, watching whether the explanation appears before the toggle moves, that canceling leaves it off, and where the caption's link lands. Then dictate with music playing. **Spends API credit; ask first.** Spec: Permissions and app lifecycle — "turning it on asks macOS for the grant immediately"
...
- Watch the step while nothing is pressed. The presets are always showing, so the card cannot resize under the page centered beneath the title. Spec: Permissions and app lifecycle — "Nothing may appear on a step that is not that decision"
...
- On a first run, look for a main window behind setup or showing past its edges, and watch what finishing opens — by **Done** and by **Skip Setup**, since they are separate buttons onto one route out. Redo Setup is the opposite case and correct — the main window is already open there, and setup sits in front of it, which is why it stayed green while a first run did not. Spec: Permissions and app lifecycle — "Setup is the only window on a first run"; Permissions and app lifecycle — "Leaving setup opens the main window"
- Walk setup as far as **Try it**, quit Scriber there, and reopen it. Setup has to come back on that step with the shortcut already working — reaching it is not finishing it, and the last step's two options have not been offered yet. Then press **Back** off the step and try the shortcut: it does nothing until you return. Spends no credit. Spec: Permissions and app lifecycle — "Setup is over when the user says so and at no other moment"; Permissions and app lifecycle — "The dictation services follow how far setup has got"
- Reach the **Setup is not finished** banner, which no ordinary route leads to: start a dictation on **Try it**, press **Back** so it cancels itself, then press **See History** on the pill. The banner has to say Scriber cannot dictate. Now return to setup, press **Continue** to arrive on **Try it** again, and switch back to the main window with ⌘\`: the same banner has to stop saying that, because the shortcut works from that step on. Spends no credit. Spec: Permissions and app lifecycle — "The dictation services follow how far setup has got"
- Press **Skip Setup** on the welcome step, read the confirmation, and press **Cancel** — setup stays where it was. Press it again and confirm: setup closes, the main window opens carrying its warning, and the menu bar offers the missing pieces rather than **Finish Setup…**. Spends no credit. Spec: Permissions and app lifecycle — "asks for confirmation first and names the way back"
- Close setup part-way with ⌘W, before its dictation step, then use the menu bar's **Finish Setup…**, then quit and reopen. That menu item is the only route back when there is no main window to hold a warning, and the reopened setup has to land on the step it was left on rather than at the welcome page. Spec: Permissions and app lifecycle — "Every launch presents onboarding until setup is complete"; Permissions and app lifecycle — "A relaunch part-way through setup returns to the step that is unmet, never past it"
...
- Take **Skip Setup** from the first step, fix everything from Settings, then **Redo Setup**. Spec: Permissions and app lifecycle — "Onboarding must be complete and the credential definitively usable"
...
- With the setting **on**, cancel a dictation three ways and listen each time: a short press canceled, a longer one canceled so **Recover canceled dictation?** appears, and Escape during transcription. All three must sound, and at the same volume as each other. Then let a dictation of a few seconds find no words: that one plays the system alert sound instead, louder, because it arrives after the transcript and so after the mute has gone. On a transcript that comes back fast it can be heard with its attack clipped — the mute's teardown runs past the 0.2s restore delay, and a cue landing in that tail loses its opening. Expected, and it goes away when the alert leaves this path. Spec: Recording and transcription — "this cue has to be Scriber's own sound"
...
- Submit audio with no recognizable words. **Spends API credit; ask first.** Spec: Recording and transcription — "clean up their temporary record and audio"
...
- Dictate with no text field focused in a native app — Calendar with nothing focused is the case. Listen for one alert sound, not two. **Turn Settings → Sound's Mute other audio while dictating off first**, or the count is zero whatever the code does: the mute is still up when this cue plays and it silences everything Scriber does not play itself. Spec: Delivery and floating pill — "Report two outcomes, never three"
