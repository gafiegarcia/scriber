...
- Start a hands-free dictation and open Settings with Command-comma while it runs. **Look before you stop it:** on General, every preset, the recorded chord, **Record**, and **Redo Setup** are greyed, and on Sound, **Check Input Level** is; none of them answers a click. Now cancel the dictation with Escape, and watch every one of them come back. Then press **Redo Setup…** and, with its confirmation on screen, start a second dictation: the box closes on its own rather than leaving a prominent button that dismisses and does nothing. Escape ends that one too. Neither dictation reaches transcription, so this spends no API credit.
