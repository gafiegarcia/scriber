...
- With the setting **on**, cancel a dictation three ways and listen each time: a short press canceled, a longer one canceled so **Recover canceled dictation?** appears, and Escape during transcription. All three must sound. Then let a dictation of a few seconds find no words — that one stays silent while the mute is up, and hearing it would mean a cue moved that should not have. Spec: Recording and transcription — "this cue has to be Scriber's own sound"
