...
- Holding `fn` shows no Cancel until the pointer moves over the pill, which widens it in; moving off narrows it back out. Escape still cancels while held with the pointer elsewhere. A quick tap of `fn` locks hands-free instead: Cancel is shown unconditionally from then on (no hover needed), and the pill widens further, animating only Confirm in on its trailing edge. Tapping `fn` again stops it. Bare `fn` still opens the emoji picker. **Wispr Flow must be quit first.**
...
- Hold the shortcut for slightly longer than a tap — around half a second — and let go. It stops and transcribes rather than going hands-free. This is the boundary `DictationShortcutTiming.tapThreshold` sets, and the only way to know whether it is set where a hand expects it.
