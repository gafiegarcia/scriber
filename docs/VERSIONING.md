...
Current native status: `0.7.0` build `7`, the locally certificate-signed login-Keychain personal-build candidate for `v0.7.0-alpha.6`. The final provisioned Data Protection Keychain state is preserved as `v0.7.0-alpha.2`.
...
- Keep the native Xcode marketing version at `0.7.0` and bundle build at `7` until the next intentionally distinguishable build.
