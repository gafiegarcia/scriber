...
## The character count is for watching, not for believing

An element qualifies as watchable when it exposes a character count, which is how a custom control with no editable role gets observed at all. That capability is also used by terminals and launchers whose input is not a writable value, so the rule stays.

It is not enough to conclude a cursor is there. A page presents an `AXGroup` reporting zero characters, no editable role and no settable selection after a text field is clicked and then left — watchable, and nowhere to paste. `TextInputTargetPolicy.acceptsAsEditor` is the stricter question, and confirmation asks it before it asks anything about what happened after the paste.

Keep the two separate. Widening `acceptsAsEditor` to accept a character count again reintroduces the false positive; narrowing `accepts` to match it stops Scriber watching controls it can currently observe.
