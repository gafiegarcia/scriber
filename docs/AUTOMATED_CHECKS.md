...
`dictation` carries a line per transcription attempt and per outcome, each stamped with a run number and elapsed milliseconds; a line per dictation start; and a pair of lines per pill resize. `dictation start` splits the shortcut press into the steps that run before the pill can be asked for: `pressToPillMs` is what the user feels, 2-20 ms in steady state and about twice that on the first press of a cold process, and the rest name the step that owns any excess. `soundAfterPillMs` sits outside that number deliberately — the start cue plays after the pill, so it is not time the press waits through. Read the resize pair together: `pill resize` is the geometry going in and `pill settled` the geometry that resulted, including `hosted`, the height of the SwiftUI content inside the glass. One line alone cannot tell a frame that was never set from one that was set and then put back, which is the difference the pair exists to show, and `hosted` disagreeing with the glass is a pill being measured against the wrong phase. Read all of this before theorising about timing or about which pill drew what: reading the source predicted the wrong cause five times where these lines answered it directly. It records run numbers, attempt numbers, outcome labels and sizes, never transcript text.
...
Three temporary lines measure how long the microphone takes to open, for **Speech at the start cue is not recorded**, and go with that item: `session open` splits the open into `teardownMs`, `makeMs`, `runMs` and `fileMs`; `mic open` gives `pressToOpenMs` and `soundToOpenMs`, the second being the window a first word spoken on the cue falls into; `first audio sample` gives `openToSampleMs`. Measured on an M4 with the built-in microphone: `runMs` 63-77 and everything else at or near zero, so the ~110 ms between cue and capture is `AVCaptureSession.startRunning` and nothing Scriber can move. A `fileMs` in the hundreds or thousands means a freshly installed bundle, not a fault: it reads 1985 on the first dictation after an install and 17-30 on the first after a reboot.
