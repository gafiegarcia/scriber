...
- Every tab still fits the window's 660x560 minimum without scrolling. Shrink Settings as far as it goes; that size is the floor the window enforces, so a tab that scrolls there scrolls at every size a user can reach.
