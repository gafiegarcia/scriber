...
- Undoing a deletion restores a retained recording, not just the row. The fixture names recordings that do not exist on disk, so the hold is only exercised by planting a file at `PendingAudio/<the fixture's audio name>` first; delete that row and the file must appear under `PendingAudio/Deleted`, and `⌘Z` must move it back and bring Retry with it. **Remove the planted file afterwards** — `--ui-testing` does not isolate `PendingAudio`, and orphan recovery would import a leftover as a genuine failed dictation in the real app. The launch purge cannot be checked this way at all: it sits behind the same `servicesAllowed` guard as the retention sweep, so no test build runs it.
