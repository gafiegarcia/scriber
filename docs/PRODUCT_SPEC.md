...
- A press too brief to have been a dictation ends in silence: no sound, no message, no history entry, no retained audio, just the pill closing. This threshold sits below the one-second recovery rule, so nothing dropped here could have been kept for a retry, and it governs every way out of a recording that short — released, cancelled by a keystroke, or failed inside the capture stack. `fn` is both the default Hold binding and the modifier half of other people's shortcuts, so a press Scriber was never meant to receive must not answer back.
...
- Report a microphone that stops sending partway through a recording. The signal threshold reads the loudest moment anywhere in a recording, so a single loud moment vouches for the whole of it, and a stream muted after capture began — macOS does this while input volume sits at its minimum — clears that check and returns a transcript quietly missing everything after the mute. Identify it by digital silence rather than by quietness, because a live microphone in a silent room still sends its own noise floor and a speaker pausing to think must never read as a failure. Say so wherever the outcome would otherwise pass as a complete dictation or blame the speaker for saying nothing, and say it in the same shape as every other input failure: the warning tone, the dwell, and the route to the input settings, since the cause is the device or its configuration and the user can only act there.
