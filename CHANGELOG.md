...
- **Settings can no longer be opened over setup.** `⌘,` and **Scriber ▸ Settings…** worked from the very first step, putting Settings over the setup window — offering the same API key field, shortcut recorder and permission rows setup was in the middle of asking for, and a **Redo Setup** button that restarts the flow underneath the window already showing it. Both are disabled while setup is on screen, first run or redo, and come back as soon as it closes or you choose **Set Up Later**.
