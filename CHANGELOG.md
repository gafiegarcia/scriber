...
- **Settings can no longer be opened over a first-time setup.** `⌘,` and **Scriber ▸ Settings…** worked from the very first step, putting Settings over the setup window — offering the same API key field, shortcut recorder and permission rows setup was in the middle of asking for, and a **Redo Setup** button for a setup that had never been done once. Both are now disabled for the whole of a first run, and come back the moment setup finishes or you choose **Set Up Later**. Redoing setup later leaves Settings reachable as before, since that is where the redo starts.
