...
- **Dictation now lands in apps you have just opened.** Dictating into a freshly opened app — Calendar's search bar was the clearest case — failed almost every time, while pressing ⌘V yourself in the same moment worked without fail. Scriber was asking the app's own **Edit ▸ Paste** menu item as a backup, and opening that menu takes the cursor out of the very field being pasted into, so by the time Paste ran there was nowhere for the text to go. It also froze Scriber for seconds against an app that was still starting up. Scriber now asks the one way your own hand does, and nothing else.
- **Scriber no longer says a dictation was delivered when it was not.** It could report success purely because a text box happened to have the cursor, without anything having actually accepted the text — silently, with no notice at all, so the only sign was your words not being there. Delivery is now confirmed by one thing: the app actually taking the transcript. Nothing else counts.
- **A dictation that fails now makes one alert sound instead of two**, because there is one paste attempt instead of two.
- **Pasting into a document no longer risks pasting what you had copied before.** Scriber puts your clipboard back after a dictation lands, and it was doing that a moment too early — while the app was still reading. Google Docs pasted the old text into a table cell about one time in five, and Raycast Notes pasted nothing at all when the old item was an image it could not accept, which looked like dictation being broken there entirely. Both are fixed by the same wait.
...
- **Every dictation is a fraction quicker.** A delay before each paste turned out to be waiting for something that had already finished.
- **A dictation that could not be pasted no longer makes a sound**, but being refused at a password field does. Forgetting to click into a text box is not something going wrong, and the notice that appears is large, tinted, and stays for five seconds. Sound is kept for the things you could not have caused — no words heard, no microphone, no sound reaching it, transcription failing.
