...
- **A dictation is no longer reported as delivered when nothing took it.** On a page with nothing focused, Scriber sometimes announced a successful paste, restored your previous clipboard, and left the transcript nowhere — it was reading the loss of the text box it had been watching as though text had arrived in it. It now requires the destination to have actually taken the transcript before saying so. Where it cannot tell — a browser slow enough to paste after Scriber has stopped waiting — it says **Copied** and keeps the transcript, rather than guessing and losing it.
...
- **A dictation that cannot be delivered says so several times sooner.** The wait before Scriber gave up was set when no destination had been timed; every one measured since answers within a tenth of a second, including the editors the long wait existed for. A failed delivery in a web page now reports in about half a second rather than close to three, and a successful one is unaffected — it never waited.
