...
- **Delete is red, the way Copy is blue.** The colour is there at rest rather than arriving on hover, so you can see which of the two you are about to hit before the pointer is on it. Pointing at either still lifts the same faint grey square behind it. A Copy button with nothing to copy stays dim and does not react at all.
...
- **Failures say what happened in plainer words.** "Could not reach ElevenLabs: The Internet connection appears to be offline." is now **No internet connection** — one sentence rather than Scriber's and the system's stitched together, and it no longer claims you are offline when you are not.
- **The Transcription failed notice stops cutting off its own message.** It was 440 points wide and needed more, so short messages Scriber writes itself lost their last words to an ellipsis — "ElevenLabs is temporarily unavailable." wanted 485. The notice is now 460, the widest it can be without moving the window, and those messages were shortened to fit it: **ElevenLabs is unavailable**, **ElevenLabs rate limit exceeded**, **No internet connection**. A message that comes back from ElevenLabs itself can still be longer than the line — the history entry holds all of it.
