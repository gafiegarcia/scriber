...
- **Installed with Homebrew? Scriber now tells you to update with Homebrew.** The update offer used to send everyone to the release page, which is the wrong move for a Homebrew install — dragging a download over it leaves Homebrew describing an app it no longer manages. Scriber recognises how it was installed and offers `brew upgrade --cask scriber` instead, with a button to copy it.
...
- **The Window menu can no longer hang Scriber.** Every Scriber window, setup included, used to be listed there, and reopening setup through it while a dictation was running rebuilt the window over and over with nothing on screen to show for it — the same freeze a Settings button was fixed against, reachable by a route that had never been. Scriber's windows no longer appear in the Window menu; reach them through the menu bar item or Command-comma instead.
- **Setup no longer carries a dictation off its last step.** Starting a dictation on **Try it!** and then pressing **Back** or **Skip** left it recording into a box that was no longer there, on steps whose controls are switched off while one runs. Leaving that step now ends the dictation. This is setup's rule alone: everywhere else in Scriber a dictation survives whatever you open.
