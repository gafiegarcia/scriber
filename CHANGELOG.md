...
- **Scriber stops working while you are not dictating.** It used to ask macOS whether it was set to launch at login every five seconds, for as long as the app was running, and each of those took 21 to 30 milliseconds of the one thread every app on the Mac draws on. Nothing asked for it and nothing needed it that often. An idle Scriber now does nothing at all.
