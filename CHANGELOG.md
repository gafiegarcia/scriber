...
Native bundle build 97, installed from an entitlement-free, locally signed
Release build.
...
- Scrolling a long dictation history is noticeably smoother. The titlebar's day
  ...
  frame.
- Times in the dictation list no longer look oddly spaced out. Forcing every
  digit to the same width padded narrow ones like `1`, which read as gaps in
  times like `15.16`. The column still lines up without it.
