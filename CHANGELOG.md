...
- **The permission and API key pills no longer cut off their own captions — they no longer have captions.** **Permissions required** carried *"Enable Accessibility so Scriber can detect global shortcuts and insert text."* in a space 102 points too small for it, and the credential pills overran by 2 and 10. The captions said what the title and the button already do: Review opens Settings on the tab naming the missing permission, and **ElevenLabs API key is missing** leaves nothing for a second line to add. Both pills are now single lines — shorter, and 52 points tall rather than 60, like every other one-liner. The full message is still in the main window's own warning, which is where it can be read at length.
