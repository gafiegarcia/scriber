...
- **Every dictation now stays on your clipboard.** Scriber used to put a transcript there only when it could not deliver it, and take it straight back off once it could — so a dictation that worked left nothing behind to paste again, and one that failed replaced whatever you had copied anyway. Dictation now behaves like pressing Command-C as well as landing at your cursor: it is on the clipboard every time, ready to paste again, and your clipboard manager keeps it. Dictation in flight is still hidden from clipboard managers; only what is left behind afterwards is plain. Nothing is put back either, which fixes a smaller annoyance — restoring a copied Finder file used to hand it back as data rather than as the file.
...
- **Scriber no longer tells you a dictation failed when it worked.** Pasting into a browser you have just opened would often land the text correctly and then claim no text box was focused — because a freshly started browser does not tell Scriber where your cursor is, and reports the same thing whether there is a cursor it has not described yet or none at all. Since those two cannot be told apart, Scriber stops pretending it knows: in a browser it now shows a brief **Copied** and leaves it there, so a dictation that arrived is not announced as lost, and one that did not is a Command-V away. The full notice — the one that hands you the transcript back — still appears wherever the answer can be trusted, which is where it is worth reading.
- **A dictation with nothing focused no longer disappears.** Click a search field on a page, click away from it, then dictate: Scriber reported the transcript delivered and it was nowhere — not in the page, and not on the clipboard either, because it used to put your previous clipboard back once it believed a paste had worked. The transcript was simply gone. Two things were wrong. Web pages read the clipboard whenever anything is pasted, whether or not there is somewhere to put it, and Scriber took that as proof; and when the text box it was watching disappeared, it read that as text having arrived. It now requires the destination to have actually taken the transcript, and the transcript stays on your clipboard regardless.
- **A dictation that cannot be delivered says so about three times sooner.** The wait before Scriber gave up was set when no destination had been timed; every one measured since answers within a tenth of a second, including the editors the long wait existed for. A dictation that could not be delivered now reports in about a second rather than close to three, and one that worked is unaffected — it never waited.
