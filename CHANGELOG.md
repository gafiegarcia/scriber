...
- **Scriber is now a download.** It is signed with an Apple Developer ID certificate and notarized by Apple, so you can install it from a disk image or with `brew install --cask gafiegarcia/tap/scriber` and open it without macOS refusing to run it. The image opens arranged, with Scriber on the left, an **Applications** shortcut on the right, and an arrow between them. Building from source is no longer the only way in.
