...
- **The menu bar no longer comes up dead when Scriber starts into setup.** With **Show in Dock** off, Scriber launched without a menu bar and took one afterwards — and macOS drew that menu bar without letting it work. Every menu was inert, the Apple menu included, until you clicked another app and came back; `⌘,` did nothing; and `⌘⇧/` answered "Help isn't available for Scriber" for as long as the app stayed open, even once the rest of the menus recovered. Scriber now decides whether it needs a menu bar before it finishes launching, so the one you get works from the start. Nothing changes about when the Dock icon appears.
