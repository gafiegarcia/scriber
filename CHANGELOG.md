...
- **The pill no longer flashes a misshapen capsule** on its way from a message like **Recover canceled dictation?** back to a one-line **Transcribing…**. Its corners used to arrive before its new size did, leaving it drawn as a stretched, not-quite-capsule for as long as that phase lasted. You rarely saw it anywhere else: a recording redraws itself ten times a second and quietly repaired the shape, and transcribing does not, which is why that was the one place it showed.
