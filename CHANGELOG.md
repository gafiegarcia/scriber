...
## 0.9.5 (build 352) — 2026-09-17

A setup and notices release. Setup no longer counts itself finished two steps early, no longer opens behind the window you cannot use, and no longer starts a dictation while you are testing the shortcut. The pills were measured against what they actually hold rather than cut off mid-sentence, and deleting a dictation is one click with `⌘Z` behind it instead of four clicks and a dialog.

### Added

- **The transcribing pill has a Cancel button.** Canceling mid-transcription was previously possible only with `Escape`. It sits exactly where the recording pill's Cancel was, so nothing shifts as a recording becomes a transcription, and it does what the key does.

### Changed

- **Deleting a dictation is one click, and `⌘Z` brings it back.** Delete used to sit behind the `···` menu and then a confirmation dialog — four clicks to remove one entry, on something you do every day, which read as obstruction rather than safety. Delete is now its own button beside Copy, it acts immediately, and the way back is the one every Mac app already has: **Edit ▸ Undo** or `⌘Z` restores the entry, its transcript, and any recording it was keeping for retry. Press it again and the one before that comes back; `⇧⌘Z` deletes it again. The `···` menu is gone — Delete was the only thing in it. Right-clicking a row still offers Copy, Retry and Delete, and none of them asks first now. Delete is unavailable on an entry while it is retrying, since its recording is in use. **Clear Dictation History still asks**, because wiping everything is not something you reach by mis-aiming — and clearing everything cannot itself be undone, though an entry you deleted beforehand keeps its own `⌘Z`.
- **The countdown ring now sits beside the close button on every notice.** On the one-line notices — **Permissions required**, the ElevenLabs credential ones, **Transcription failed**, **No words detected**, **No sound from the microphone** — it used to sit to the left of the buttons, so the thing counting down was separated from the ✕ it was counting down to. The taller notices already had it beside the ✕; now all of them do.
- **Delete is red, the way Copy is blue.** The color is there at rest rather than arriving on hover, so you can see which of the two you are about to hit before the pointer is on it. Pointing at either still lifts the same faint gray square behind it. A Copy button with nothing to copy stays dim and does not react at all.
- **The short pills are 76 points narrower and the recording one no longer says the word "Recording".** They were 280 points wide; they are now 204. The recording pill shows the elapsed time and the level meter — a red meter running beside a stopwatch in Scriber's own pill did not need the label — and the meter now takes whatever width Cancel and Confirm are not using rather than being a fixed 58 points: it roughly doubles, to 118, while the pointer is away, and narrows to 44 in hands-free where both controls share the row with it. Cancel and Confirm sit on the pill's curve rather than slightly inside it. VoiceOver is still told it is a recording, and reads the elapsed time when you ask for it rather than announcing it every second.
- **A dictation that fails before it is saved now tells you what went wrong, and stops offering buttons that did nothing.** When the microphone would not open, the recording would not finish, or the entry itself would not save, Scriber showed **Transcription failed** with **Retry** and **See History** — but no history entry existed, so Retry did nothing at all and See History had nothing to show. Those failures now read **Dictation failed** in a box rather than a one-line pill, carry no buttons, and print the whole of what went wrong instead of one truncated line, selectable so you can copy it into a bug report. Where the words are macOS's, you now get all of them: it answers a recording it cannot start with just "Cannot Record" and leaves the explanation in a second sentence, which Scriber was throwing away. Where the words were Scriber's, they no longer read like a debugger — "No recording is active." and "The microphone recording did not finish." are now "The recording ended before it could be saved." and "The recording did not finish saving." **Transcription failed** is unchanged and still offers both buttons; those failures do have an entry waiting, and it holds the full message.
- **The copied-result box is 50 points narrower**, at 430, and the transcript inside it sits a little further below the line above. It shares its width with the new failure box: the two appear in the same place minutes apart, and at different widths the second read as the pill having moved rather than as a different notice.
- **Clicking a pill's surface no longer does anything.** Seven pills used to act on a click landed anywhere on the glass, away from any button — opening the main window, a Settings tab, or macOS Sound settings, or dismissing themselves. Nothing said they would. The pill floats over whatever you are working in, where a click aimed past it is easy to make, so it now acts only through the buttons you can see, `Escape`, and the dictation shortcut.
...
- **Scriber's spelling is American throughout.** The app read British in places — "Cancelled", "No recognisable words" — against a Mac whose own interface is American, and it had started disagreeing with itself: the pill offered to recover a "canceled" dictation while History labeled that same dictation "Cancelled". Entries already in your history keep the wording they were saved with.
- **Every dictation sound but the start and cancel cues is now your own alert sound.** Scriber played your alert sound for failures but a hard-coded Tink for a cancellation or a transcript that landed on the clipboard. Tink is the macOS default, so on most Macs the two were the same sound at two different volumes — but if you had chosen a different alert sound, Scriber played Tink at you anyway, which is very often the exact sound you had chosen away from. A transcript landing on the clipboard, a failure, no signal and no words now all play whichever alert sound you have set, and silencing the system alert silences them. **Canceling keeps Tink**, because it is the one ending that happens while **Mute other audio while dictating** is still holding the rest of your Mac quiet — and that mute silences anything Scriber does not play itself, so a cancellation asking macOS for your alert sound was heard as nothing at all. Scriber will have cues of its own here in a later version.
- **The sounds Scriber plays itself are louder.** The start and cancel cues sat at just over half volume, which put them well under the system alert sound the other endings use — on a Mac where both are Tink, the same sound arrived at two noticeably different levels. They are closer now. The alert sound is still governed by **Alert volume** in System Settings → Sound, which is yours to set and which Scriber does not touch.

### Fixed

- **Coming back to Scriber mid-setup brings back setup, and nothing else.** If you closed the setup window before finishing and then reopened the app — from the Dock, or by launching it again — Scriber opened the main window, which then reopened setup *behind* it. You arrived at a window you cannot really use, with the one you needed hidden under it. Reopening now asks the same question a launch does and gives you setup on its own until it is finished.
- **Quitting during setup no longer skips the rest of it.** Setup used to count itself finished the moment you reached **Try it** — two steps early, because that is where the shortcut has to start working — so quitting there and reopening Scriber gave you the main window and never showed setup again. You never saw the last step, or the two options on it. Setup is now over when you say it is, by **Done** or by **Skip Setup**, and coming back part-way returns you to the step you were on with your shortcut already working.
- **"Set Up Later" is now "Skip Setup", and it asks first.** The old name promised something the button did not do: it did not set up later, it dismissed setup for good. The new one says what happens, and confirms before it does — telling you where setup went, since Scriber will not offer it again on its own. Everything it configures is still in Settings, and Scriber still points at whatever is missing from the menu bar and the main window.
- **Finishing setup now opens Scriber's window, including the very first time.** On any launch that began in setup — a genuine first run, or coming back to a setup you had closed unfinished — both ways out of setup closed it onto nothing at all. Scriber stayed running in the menu bar, but the window you had just finished setting up never appeared, and the Dock icon went with it. Redoing setup from an already-running Scriber was unaffected, which is why this went unseen.
- **Settings opens in sessions that never showed the main window.** If Scriber started straight into setup — because you closed setup unfinished and came back — `⌘,` and **Scriber ▸ Settings…** did nothing at all, silently, for the rest of that session. Settings could only be created by the main window, which in that session never existed. It no longer waits for one.
- **The menu bar no longer comes up dead when Scriber starts into setup.** With **Show in Dock** off, Scriber launched without a menu bar and took one afterwards — and macOS drew that menu bar without letting it work. Every menu was inert, the Apple menu included, until you clicked another app and came back, and `⌘,` did nothing. Scriber now decides whether it needs a menu bar before it finishes launching, so the one you get works from the start. Nothing changes about when the Dock icon appears.
- **Redoing setup starts at the beginning again.** It picked up wherever the previous run had reached — usually **Try it** — still holding that run's answers, because the setup window is reused rather than rebuilt. Redo Setup now builds a fresh one. Closing setup unfinished and reopening still resumes where you left off, which is a different thing to ask for.
- **Setup's shortcut test is quieter, and honest about what it has seen.** The big key cap lit up the moment you held the modifiers — `⌃⌥` lit it for `⌃⌥D`, before the letter — so it looked like the key had registered when it had not, and it then stayed lit until you released the modifiers rather than the key. It now lights for the whole chord and goes dark as you let it go. Pressing your shortcut there also made macOS play your alert sound, because nothing in setup was answering that keypress; the test takes it now. A near miss still sounds, which tells you the chord did not land.
- **Testing your shortcut during setup no longer starts a dictation behind the window.** Setup switches the shortcut on as you move from the shortcut step into **Try it**, and going **Back** left it on — so pressing the key to test it started a real dictation behind the setup window. With a shortcut that has a letter in it, it was worse than untidy: the keypress was swallowed before the step could see it, so the test never passed and **Continue** stayed greyed out, trapping you on the step unless you rebound to something else. The shortcut now does nothing on any step before **Try it**, where it is meant to be tried, and any dictation still running ends as you arrive.
- **The `···` menu's Delete could stop responding, then produce a stack of dialogs minutes later.** Deleting several entries quickly could leave Delete doing nothing at all — on that row and on every other row too — until some unrelated click woke the window up, at which point every suppressed confirmation appeared one after another, none of them saying which entry it belonged to. It is gone with the dialog itself.
- **A row's icons now accept clicks everywhere they light up.** The gray square that appeared under the pointer was 26 points across while only the 16-point glyph inside it took the click, so a five-point ring highlighted and did nothing.
- **Switching off your microphone mid-dictation no longer loses everything you had said.** Disconnecting a Bluetooth input while recording used to leave the timer counting beside a frozen waveform, and letting go then reported **Dictation failed / The recording ended before it could be saved.** — with nothing kept. The recording had in fact survived on disk the whole time; it just went unnoticed until the next launch, which quietly recovered it into History. Now the dictation ends the moment the device does, and an amber **Microphone disconnected** panel offers **Recover**, the same button a canceled dictation gets, which transcribes what was recorded and pastes it where your cursor is. It stays up for eight seconds, because it appears while you are still mid-sentence. Let it go and the dictation is waiting in History, as any other kept recording is. Where the recording was too short or too quiet to be worth anything, it says only **Microphone "…" disconnected** — no button, because there is nothing behind one.
- **Pressing the shortcut at the moment a microphone is disconnecting no longer answers with two words of macOS.** A device that is going away is still listed for a second or so after it stops being openable, and Scriber showed AVFoundation's own **Cannot Record** for it. It now reads **Microphone "…" is unavailable**, the same as when the device has already gone.
- **Reconnecting a microphone no longer restarts an input test it had killed.** Unplug the device the input meter is watching and the test stops with **The selected microphone is unavailable**, as it should — but plugging the device back in used to reopen the microphone and carry on metering, without being asked. Press **Check Input Level** again when you want it back.
...
- **The permission and API key pills no longer cut off their own captions — they no longer have captions.** **Permissions required** carried *"Enable Accessibility so Scriber can detect global shortcuts and insert text."* in a space 144 points too small for it, and the two credential captions overran by 18 and 26. The captions said what the title and the button already do: Review opens Settings on the tab naming the missing permission, and **ElevenLabs API key is missing** leaves nothing for a second line to add. Both pills are now single lines, 52 points tall rather than 60 like every other one-liner, and they share one width so that meeting the second right after the first does not read as the pill having moved. The full message is still in the main window's own warning, which is where it can be read at length.
- **Three pills no longer cut off what they were trying to tell you.** The one naming an unavailable microphone, **No words detected**, and **Transcription failed** were each given a width that had never counted the controls sharing their row, so their text ran out of room. Each is now measured against what it actually holds. **Transcription failed** needed more than width: even at 460 points — the widest a pill can be without moving the window — some of the messages Scriber writes for it were too long, so they were shortened to **ElevenLabs is unavailable** and **ElevenLabs rate limit exceeded**, and **Not connected to the Internet** became **No internet connection**. A message that comes back from ElevenLabs itself can still be longer than the line; the history entry holds all of it.
...
- **The dictation history loads as you scroll it** rather than building every row at once, so scrolling a long history is smooth. With the main window open a dictation costs about a third less than it did.
- **The start cue is Frog rather than Blow.** A shorter, sharper sound, so the moment you hear the dictation begin is the moment it began.
- **Pressing the shortcut in the instant a dictation is being delivered is ignored**, the way a press during transcription already was. Delivery outlasts the pill by a fifth of a second, and longer when it fails, and a dictation starting inside that window raced the one still finishing.
...
- **A held dictation can no longer be left recording with nothing able to stop it.** Starting a new hold at the exact moment the previous transcript landed could lose the release, so letting go did nothing and the recording ran on until Scriber was force-quit. It took deliberate timing to hit, but it was reachable by anyone dictating quickly.
- **Scriber now does nothing at all while you are not dictating.** It used to ask macOS whether it was set to launch at login every five seconds, for as long as the app was running, and each of those took 21 to 30 milliseconds of the one thread every app on the Mac draws on. Nothing asked for it and nothing needed it that often.
- **A dictation costs a fraction of what it did when the main window is closed.** Finishing one used to spend about 370 milliseconds rebuilding the whole history list — filtering every dictation you have ever made, twice, then regrouping all of them by day — for a window that was not even on screen. Delivery was stuck behind that work, which made it look slow when it was not: it now completes in 33 to 46 milliseconds rather than about 150, and a shortcut pressed straight after a dictation is picked up almost at once instead of waiting up to a third of a second.
...
- **Installed with Homebrew? Scriber now tells you to update with Homebrew.** The update offer used to send everyone to the release page, which is the wrong move for a Homebrew install — dragging a download over it leaves Homebrew describing an app it no longer manages. Scriber recognizes how it was installed and offers `brew upgrade --cask scriber` instead, with a button to copy it. The menu bar's update item takes you to that command in Settings rather than to a download, so neither place can send a Homebrew install the wrong way.
...
- **Canceling a dictation went silent, and threw away audio you could have recovered.** Every cancel was being treated as a slipped finger: no sound, no **Canceled** message, and no retryable recording kept — even for a long dictation with speech in it. Typing during a held dictation also discarded it at any age, rather than only in the first second where a stray key is a mistake.
...
- Canceling a dictation no longer looks like something went wrong. The recovery pill drops the amber tint and the warning triangle and reads in the app's plain colors; **Undo** is still there if you want the recording back.
...
- Holding a shortcut bound to a key, such as `⌘⇧D`, no longer stalls the Mac. macOS repeats a held key about eleven times a second, and Scriber read each repeat of the chord you were holding as you starting to type — which canceled the recording and let the next repeat start a new one, several times a second, with all of it running inside the callback the whole system's keyboard and mouse input waits on. A held Hold or Toggle chord is now one press and one release, and none of the work it triggers happens inside that callback. A modifier-only shortcut such as the default `fn` never showed this.
...
- The pinned day label lost its outlined capsule. It now sits flat against the page in the same color, so entries scrolling past simply disappear under it instead of sliding behind a visible shape.
...
- The setup window no longer opens partly off the bottom of the screen, under the Dock. It is centered and sized to the display every time it appears, and its steps scroll if they ever do not fit.
- A microphone that produces no sound at all — muted, wrong device, input volume at zero — now says so, instead of the dictation vanishing with no transcript, no history entry, and no warning. It is reported separately from a recording that did carry sound but produced no recognizable words.
