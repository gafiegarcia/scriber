...
- **The short pills are 18 points narrower.** Recording, **Transcribing…**, **Copied** and **Canceled** all sat on one shared minimum width that had not been revisited since the word "Recording" was taken off the recording pill, so most of them carried empty space between their text and whatever sat on the right. The level meter gives up six of its bars in hands-free, where Cancel and Confirm share the row with it, and is wider than before everywhere else.
- **A dictation that fails before it is saved now tells you what went wrong, and stops offering buttons that did nothing.** When the microphone would not open, the recording would not finish, or the entry itself would not save, Scriber showed **Transcription failed** with **Retry** and **See History** — but no history entry existed, so Retry did nothing at all and See History had nothing to show. Those failures now read **Dictation failed**, carry no buttons, and print the whole of what macOS reported instead of one truncated line. macOS answers a recording it cannot start with just "Cannot Record" and leaves the explanation in a second sentence, which Scriber was throwing away; that sentence is now shown too, and the text can be selected and copied into a bug report. **Transcription failed** is unchanged and still offers both buttons — those failures do have an entry waiting.
- **A dictation that fails now says what happened to your dictation, not what the recorder found.** "No recording is active." and "The microphone recording did not finish." were the recorder's own vocabulary showing through; they now read "The recording ended before it could be saved." and "The recording did not finish saving."
- **The two message boxes are the same width.** The copied result was 480 points wide and the new failure box 430. They appear in the same place minutes apart, and at different widths the second read as the pill having moved rather than as a different notice.
