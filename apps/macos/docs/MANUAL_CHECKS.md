...
- `Fn-Space` locks hands-free, with Cancel on the pill's leading edge and Confirm
  on its trailing edge. Converting a held recording to hands-free widens the
  existing pill and animates both controls in; Cancel spends no credit. Hold is
  ignored while locked, and Toggle stops it. Bare `Fn` still opens the emoji
  picker and types a normal Space. **Wispr Flow must be quit first.**
