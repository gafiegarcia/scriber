...
- Group Dictation history by local calendar date and vertically center each entry's time beside its transcript content. Keep the page centred, with one transparent continuously rounded card per day, a subtle outline, and row separators that span the card's full width. Align each day label to its card and pin it while that day is on screen. Back it with a flat fill of the page's own colour — the page paints that colour explicitly so the two cannot drift apart — so a row scrolling under the label simply stops being visible. Neither a gradient nor a translucent fill: the first fades out inside the label's own glyphs, the second lets rows ghost through it.
