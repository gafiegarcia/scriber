...
- Confirm observable target mutation when Accessibility exposes it. For editors that hide their text state, observe application-wide Accessibility text mutations and, after the destination requests the promised pasteboard text, require either inserted-text metadata matching the transcript or a selection mutation on an editable text element. Never treat a dispatched Paste command or clipboard read alone as proof of insertion.
