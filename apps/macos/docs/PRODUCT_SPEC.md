...
- Confirm observable target mutation when Accessibility exposes it. For editors that hide their text state, observe application-wide Accessibility value changes and require inserted-text metadata matching the transcript after the destination requests the promised pasteboard text. Never treat a dispatched Paste command or clipboard read alone as proof of insertion.
