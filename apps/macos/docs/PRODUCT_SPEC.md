...
- Name the day currently at the top of Dictation history in a strip inside the window's titlebar, below the toolbar items, sitting 2pt inside the cards' leading edge — clear of the edge without reading as an indent of its own, so the label and the card line up and only the row content inside the card steps in. A Settings section header sits the same way against its card. It is the only place a day is named; the list itself carries no day label. Give the strip no backdrop of its own — it shares the titlebar's, and a separate surface beneath the toolbar cannot be made to match it. Hand the name over to the next day as that day's card reaches the titlebar, and collapse the strip whenever no day is on screen.
