...
- Every unmet permission offers exactly one action, labelled `Allow`, worded the same in Settings and in onboarding. The title never reports what macOS has recorded so far; whether the click raises a system prompt, opens the matching System Settings pane, or does both is Scriber's business. That action must always advance the grant — a button that cannot change anything must not be shown.
