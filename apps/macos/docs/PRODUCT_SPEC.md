...
- Recovery UI may offer See History, Undo, Retry, Check Input, Review, Update Key, or View Usage according to the failure state. One destination keeps one name across every pill that reaches it.
- Tint the pill by what the outcome was, from the same tone vocabulary the window's toast stack uses, so the two surfaces cannot colour one outcome two ways. A dictation in flight is not an outcome and carries no tint. Tint, never fill: the pill floats over the user's own work and must still read as glass.
