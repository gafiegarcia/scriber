...
- The pill's window reserves the widest layout a recording can need, so it neither resizes nor recentres for the rest of the recording; the capsule inside it changes width instead, and one animation engine owns that width so that nothing inside the pill interpolates against a second one. Every point of that window outside the capsule must fall through to the application underneath.
