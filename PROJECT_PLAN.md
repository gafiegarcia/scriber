...
- Tightened ElevenLabs credential handling. New and updated Keychain items now receive an explicit calling-app-only access list, and existing items are migrated from the previous unrestricted access setting when Scriber Dictate next reads them. API-key save/update now first performs ElevenLabs' documented, non-generative `GET /v1/models` authentication request, shows a Checking state, reports invalid/restricted/rate-limited/service errors inline, and writes to Keychain only after a successful response. Parser validation, core type-checking, all 15 credit-free tests, and the unsigned Xcode 27 beta Debug build pass. The Keychain Access ACL display and a real valid/restricted key remain manual checks; automated tests never send a credential or contact ElevenLabs.
