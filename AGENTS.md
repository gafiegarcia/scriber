...
- Inline comments are *only for `Known and unfixed:` notes*. The code itself plus `docs/PRODUCT_SPEC.md` are the source of truth. Do not explain what something *used to be* (that's git history). Comments other than `Known and unfixed:` notes need my explicit permission and usually not useful (except for decisions that the code cannot represent, absolutely cannot live in the docs instead, and absolutely needed for future reference).
